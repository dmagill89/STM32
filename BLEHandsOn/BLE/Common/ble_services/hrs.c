                ...
                case EVT_BLUE_GATT_WRITE_PERMIT_REQ: {
                    ...
                    if (write_perm_req->attr_handle == (HRS_Context.ControlPointCharHdle + 1)) {
                        ...
                        if (write_perm_req->data[0] == HRS_CNTL_POINT_RESET_ENERGY_EXPENDED) {
                            ...
                            aci_gatt_write_response(write_perm_req->conn_handle, write_perm_req->attr_handle, 0x00, /* write_status = 0 (no error))*/
                             ...
                             */
//TODO              HRS_Notification(HRS_RESET_ENERGY_EXPENDED_EVT);
                        } else {
                            /* received value of HRM control point char is incorrect */
                            aci_gatt_write_response(write_perm_req->conn_handle, write_perm_req->attr_handle, 0x1, /* write_status = 1 (error))*/
                ...
                case EVT_BLUE_GATT_ATTRIBUTE_MODIFIED: {
                    ...
                    if (attribute_modified->attr_handle == (HRS_Context.HeartRatemeasurementCharHdle + 2)) {
                        ...
                        if (attribute_modified->att_data[0] & COMSVC_Notification) {
                            ...
                            // TODO HRS_Notification(HRS_NOTIFICATION_ENABLED);
                        } else {
...
static tBleStatus Update_Char_BodySensorLocation(HRS_BodySensorLocation_t *pBodySensorLocationValue) {
    ...
    return_value = aci_gatt_update_char_value(HRS_Context.HeartRateSvcHdle, HRS_Context.BodySensorLocationCharHdle, 0, /* charValOffset */
    1, /* charValueLen */
...
static tBleStatus Update_Char_Measurement(HRS_MeasVal_t *pMeasurement) {
    ...
    return_value = aci_gatt_update_char_value(HRS_Context.HeartRateSvcHdle, HRS_Context.HeartRatemeasurementCharHdle, 0, /* charValOffset */
...
void HRS_Init(void) {
    ...
    hciCmdResult = aci_gatt_add_serv(UUID_TYPE_16, (const uint8_t*) &uuid,
    PRIMARY_SERVICE, 4
#if (BLE_CFG_HRS_BODY_SENSOR_LOCATION_CHAR != 0)
            + 2
#endif
    ...
    UUID_TYPE_16, (const uint8_t*) &uuid,
#if (BLE_CFG_HRS_ENERGY_EXPENDED_INFO_FLAG != 0)
        2 /** Energy Expended Info */
#endif
#if (BLE_CFG_HRS_ENERGY_RR_INTERVAL_FLAG != 0)
    ...
    hciCmdResult = aci_gatt_add_char(HRS_Context.HeartRateSvcHdle,
    UUID_TYPE_16, (const uint8_t*) &uuid, 1, /* bytes */
    CHAR_PROP_READ,
    ATTR_PERMISSION_NONE,
    GATT_DONT_NOTIFY_EVENTS, /* gattEvtMask */
    10, /* encryKeySize */
