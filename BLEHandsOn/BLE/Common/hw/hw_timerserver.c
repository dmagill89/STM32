...
/* Private typedef -----------------------------------------------------------*/
typedef enum {
    TimerID_Free, TimerID_Created, TimerID_Running
} TimerIDStatus_t;

typedef enum {
    SSR_Read_Requested, SSR_Read_Not_Requested
} RequestReadSSR_t;

typedef enum {
    WakeupTimerValue_Overpassed, WakeupTimerValue_LargeEnough
...
void HW_TS_Init(HW_TS_InitMode_t TimerInitMode, RTC_HandleTypeDef *hrtc) {
     ...
     */
    localmaxwakeuptimersetup = (((SynchPrescalerUserConfig - 1) * AsynchPrescalerUserConfig) >> WakeupTimerDivider)
